...
class baseModel extends ModelData{
  ...
  public function prepareDefaultSelectElements(){
    ...
    foreach($this->tb_columns->getColumnsMetadata() as $fname => $tbcolobj){
      ...
      if($tbcolobj->getFieldDetails()->canShow()){
        $this->_ADD_SEL_FIELD( $tbcolobj->getField(DBFactory::getInstance()->getDB()->prefixTB()) );
      }
      if($tbcolobj->getFieldDetails()->getOrder()!=null){
        $this->_ORDER_BY( $tbcolobj->getField(DBFactory::getInstance()->getDB()->prefixTB()), $tbcolobj->getFieldDetails()->getOrder() );
      }
  ...
  public function getSelectStatement(){
    ...
    return DBFactory::getInstance()->getDB()->getSelectStatement( $this->getTableName(), $this->SelectionElements, $this->WhereConditionElements, $this->SelectionOrderElements );
  }

  public function getUpdateStatement(){
    return DBFactory::getInstance()->getDB()->getUpdateStatement( $this->getTableName(), $this->UpdateElements, $this->WhereConditionElements );
  }

  public function getDeleteStatement(){
    return DBFactory::getInstance()->getDB()->getDeleteStatement( $this->getTableName(), $this->WhereConditionElements );
  }
      ...
      if($values!=null){
        ...
        foreach($values_local as $value){
          if($this->getTableColumn($keys[$idxkey])->getFieldDetails()->getDefault()!=null and
             ...
             if($idxkey == 0){
               $this->_CONDITION( $this->getTableColumn($keys[$idxkey++])->getField( DBFactory::getInstance()->getDB()->prefixTB() ) , "=", Token::getInstance()->getId() );
             }else{
               $this->_AND( $this->getTableColumn($keys[$idxkey++])->getField( DBFactory::getInstance()->getDB()->prefixTB() ) , "=", Token::getInstance()->getId() );
             }

          }
          if($idxkey == 0){
            $this->_CONDITION( $this->getTableColumn($keys[$idxkey++])->getField( DBFactory::getInstance()->getDB()->prefixTB() ) , "=", $values_local[$idx++] );
          }else{
            $this->_AND( $this->getTableColumn($keys[$idxkey++])->getField( DBFactory::getInstance()->getDB()->prefixTB() ) , "=", $values_local[$idx++] );
          }
        ...
        for($i=0;$i<sizeof($keys);$i++){
          if($this->getTableColumn($keys[$i])->getFieldDetails()->getDefault()!=null and
             ...
             //if($idx == 0){
               $this->_CONDITION( $this->getTableColumn($keys[$i])->getField( DBFactory::getInstance()->getDB()->prefixTB() ) , "=", Token::getInstance()->getId() );
             //}else{
             //  $this->_AND( $this->getTableColumn($keys[$i])->getField( DBFactory::getInstance()->getDB()->prefixTB() ) , "=", Token::getInstance()->getId() );
             //}
          }
        }
      }
  ...
  public function prepareInsertStmt( $fields_data, $key_values ){
    ...
    foreach($this->tb_columns->getColumnsMetadata() as $fname => $tbcolobj){
      ...
      if($tbcolobj->getFieldDetails()->isKey() and $tbcolobj->getFieldDetails()->canInsert()){
        ...
        $ie = new InsertUpdateElement($tbcolobj->getField( DBFactory::getInstance()->getDB()->prefixTB() ) ,$value);
        ...
        if($tbcolobj->getFieldDetails()->getDefault()->getType() == FieldDefault::type_FUNCTION){ //MySQL function call
          ...
          $this->InsertionElements[] = new InsertUpdateElement( $tbcolobj->getField( DBFactory::getInstance()->getDB()->prefixTB() ), $tbcolobj->getFieldDetails()->getDefault()->getValue() );

        }elseif($tbcolobj->getFieldDetails()->getDefault()->getType() == FieldDefault::type_TOKEN_ID){//JSON WebToken id
          $token = Token::getInstance();
          if($token->getId()!=null){

            $this->InsertionElements[] = new InsertUpdateElement( $tbcolobj->getField( DBFactory::getInstance()->getDB()->prefixTB() ), $token->getId() );
          ...
          $modelObj->_PREPARE_SELECT()->_ADD_MAX( $modelObj->getTableColumn( $tbcolobj->getFieldDetails()->getDefault()->getSubQueryMaxField() )->getField( DBFactory::getInstance()->getDB()->prefixTB() ) , "LastKey" );
          ...
          foreach($keys as $key){
            ...
            if($firstCondition){
              $modelObj->_CONDITION($modelObj->getTableColumn($key)->getField( DBFactory::getInstance()->getDB()->prefixTB() ), "=", $fvalue );
              $firstCondition = false;
            }
            else{
              $modelObj->_AND($modelObj->getTableColumn($key)->getField( DBFactory::getInstance()->getDB()->prefixTB() ), "=", $fvalue );
            }
          }
          ...
          $newKey = $lastKey + 1;
          $this->InsertionElements[] = new InsertUpdateElement( $tbcolobj->getField( DBFactory::getInstance()->getDB()->prefixTB() ), $newKey );
    ...
    return DBFactory::getInstance()->getDB()->getInsertStatement( $this->getTableName(), $this->InsertionElements );
  ...
  public function prepareUpdateStmt( $fields_data, $keys, $values){
    ...
    foreach($this->tb_columns->getColumnsMetadata() as $fname => $tbcolobj){
      ...
      if($tbcolobj->getFieldDetails()->canUpdate()){
        ...
        $ue = new InsertUpdateElement($tbcolobj->getField( DBFactory::getInstance()->getDB()->prefixTB() ) ,$value);
        $this->UpdateElements[] = $ue;
      }elseif($tbcolobj->getFieldDetails()->getDefault() != null and
                $tbcolobj->getFieldDetails()->getDefault()->fillOnUpdate() ){
        if($tbcolobj->getFieldDetails()->getDefault()->getType() == FieldDefault::type_FUNCTION){ //MySQL function call

          $this->UpdateElements[] = new InsertUpdateElement( $tbcolobj->getField( DBFactory::getInstance()->getDB()->prefixTB() ), $tbcolobj->getFieldDetails()->getDefault()->getValue() );
          ...
          if($token->getId()!=null){
            ...
            $this->UpdateElements[] = new InsertUpdateElement( $tbcolobj->getField( DBFactory::getInstance()->getDB()->prefixTB() ), $token->getId() );
